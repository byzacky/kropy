...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setDescription('Blacklist a server from using commands.')
    ...
    async execute(interaction) {
        ...
        if (BLACKLIST.has(serverId)) {
            return interaction.reply({ content: '❌| This server is already blacklisted.', ephemeral: true });
        }

        BLACKLIST.add(serverId);
        saveBlacklist(); // Save updated blacklist
        await interaction.reply(`✅ | Server with ID ${serverId} has been blacklisted.`);
    },
